...
public class MaxRangeAnimation : MonoBehaviour {
	...
	public void StartUp()
	{
        ...
        if (active)
        {
            ...
            number = (int)Mathf.Round(maxRange * 2f * Mathf.PI / interval);
            if (number < 1)
                number = 1;
            ...
            dot.localPosition = new Vector3(GetX((float)(angle * (float)0), maxRange), GetY((float)(angle * (float)0), 0));
            ...
            for (int i = 1; i < number; i++)
            {
                ...
                nextDot.localPosition = new Vector3(GetX((float)(angle * (float)i), maxRange), GetY((float)(angle * (float)i), maxRange), 0); // Position on an imaginary circle around the parent
            }

            animationSpeed = animationSpeed / (maxRange * 2f * Mathf.PI);
        } else
        {
			// Remove particle
